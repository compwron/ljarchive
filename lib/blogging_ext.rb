...
#          "/archives/#{year}/#{Date::ABBR_MONTHNAMES[month]}", # path
#          :binary => false
#        )
#      end
    end
  ...
  def link_for_tag2(tag, base_url)
        %[<a href="#{h base_url}#{h tag}/" rel="tag">#{h tag}</a>]
  end

  def tags_for2(item, params = {})
    ...
    if item[:tags].nil? || item[:tags].empty?
        ...
        item[:tags].map { |tag| link_for_tag2(tag, base_url) }.join(separator)
    end
  end
